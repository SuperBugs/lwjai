 ...
 *   `draft`     **第三方工具替你推成草稿**，发布那一下你在平台自己的编辑器里点
 *   `manual`    **没有官方口子** —— 出图 ＋ 出文案，最后一下人自己按
 *   `retired`   **停用了**，而且说得出为什么
 *
 * ⚠ 最后一档最容易被省成"从表里删掉"，而删掉的代价是：
 *   「这个站不发某个平台了」和「那块坏了」在屏幕上字节级相同，
 *   半年后没人记得起为什么没有它。停用的默认做法是**留在表里带着日期和原因**。
 *   【2026-09-23】小红书那一行是个例外：用户明确说「小红书不要了」，整行删了，
 *   原因转记在 docs/engineering-notes.md 里。这一档留着给下一个停用的平台用。
 * ⚠ `draft` **不许写成 `official`**：它走的是一个开源扩展、用你的登录态去调平台编辑器
 *   自己的接口 —— 平台要拦照样能拦。写成 `official` 就是把第三方工具冒充成平台给的口子。
 *
 * ## ⚠ 仍然**没有「替你按发布」的非官方那一档**
 *
 * 【2026-09-22 立的】拿 cookie / 无头浏览器**替人按发布**正是小红书那次被封的形状。
 * 【2026-09-23 用户听过这句之后决定用 Wechatsync 推雪球】这不是推翻那一条 ——
 * `draft` 这一档和被封的那个形状差着三件事，缺一件都不许进这一档：
 *   ① **结构上只能存草稿**（读过适配器源码，只有 `draft/save.json`，没有发布接口）；
 *   ② 跑在**你真的浏览器**里，不是无头的；
 *   ③ **一次点一组、人来点**，不跨组排队：放不下时这一组会分成几篇**串行**推，雪球退回来的
 *      截短重推也算在这一次里、有上限（`xueqiuArticle.ts` 的 `XUEQIU_MAX_SHRINKS`）。
 *      【2026-09-23】原来写的是「一次一篇」—— 分开推（用户定的）之后一次点击可能推出
 *      一组里的几篇，那几个字已经不准了；意思没变：**每一组都要人点一次，没有替人跨组排的队**。
 * 哪天有个工具能替你直接发布，它**不许**放进 `draft` —— 那是另一档，得先回来想清楚。
 *
 * ## ⚠ 有几个数是**猜的**，标着 `calibrated: false`
 *
 * **推测不许冒充确认**（最早立在小红书发帖那条的 `SUCCESS_SIGNALS` 上，那一套 2026-09-23 删了）。
...
/** 这个平台上，最后按下发送那一下是谁做的。 */
export type PostMode = "official" | "draft" | "manual" | "retired";
...
export const SOCIAL_PLATFORMS: readonly SocialPlatform[] = [
  {
    id: "xueqiu",
    name: "雪球",
    // 【2026-09-23 用户要的】从 manual 升上来，理由见文件头「仍然没有…」那一节。
    mode: "draft",
    modeWhy:
      "雪球没有公开的发帖接口。长文走「文章同步助手」（Wechatsync）扩展：" +
      "它用你 Chrome 里的雪球登录态，只存草稿（读过它的源码，只调 draft/save.json），" +
      "发布那一下你在雪球编辑器里点。它不是官方接口，雪球要拦照样能拦。" +
      "上面那段短帖仍然是手动粘的。",
    // 雪球是内容平台，帖子里带外链是常态。
    urlOnCard: true,
    urlInBody: true,
    ...
    symbolLimit: 3,
    /**
     * 【2026-09-23 雪球服务端的原话】「输入标题太长，请确认不超过50个字」
     * （推长文到草稿箱那条路，AAPL 那一组三篇全被这一句退回）。
     * ⚠ 帮助页写的是「标题无限制」，这一格原来据此留空 —— **服务端说了算**。
     *   这是对着真回话核过的数，不是猜的，所以 `calibrated` 仍是 true。
     */
    titleMax: 50,
    calibrated: true,
  ...
  // 【2026-09-23 用户要的】小红书那一行（retired）整行删了 —— 原话「小红书不要了」。
  // 为什么停、哪天要重开得先守住什么，记在 docs/engineering-notes.md socialPlatforms 那一条，不在页面上了。
];
