...
基于 AI 智能体（AI Agent）的美股研究发布系统。将 ChatGPT、Claude、Gemini 等大语言模型（LLM）生成的
个股研究、问答与提示词发布为静态站点：支持在本地后台撰写或导入模型输出，逐条标注来源（智能体与模型），
同一问题的多智能体结果并列对比，经发布闸校验后部署至 Cloudflare。
...
![研究列表](docs/screenshots/research-list.jpg)

研究详情：来源标注、同题切换、目录与正文。

![研究详情](docs/screenshots/research-detail.jpg)
...
- 来源标注：记录生成内容的 AI 智能体与模型（ChatGPT、Claude、Gemini、Codex 等）；人工撰写、未标注分别显示
- 多智能体对比：同一问题或选题的多份结果合并展示，可在详情页切换
