...
/**
 * 图片卡片的**按内容缓存** —— `/_cards` 打开一次要画十几张，每张 satori 一遍。
 * （文件名还叫 xhs 是历史原因：卡最早是给小红书做的，那一套 2026-09-23 删了，卡留着。）
 *
 ...
 * ★ 顺带保住了一条性质：`/_cards` 页上那张、`/_share` 上「图片卡片」链接给的那张，
 *   走的是同一条接口、同一份缓存 —— **你下载的就是你在页面上看过的那张图**，逐字节相同。
 * ⚠ 平台只影响地址那一格（`urlOnCard`），它就在白名单里、会进指纹：两个平台的规矩不一样时，
 *   各自是各自的一张（下面按前缀清旧图时会互相清掉，最坏是多画一次，不会给错图）。
 *
 * ## ⚠ 缓存落在 `node_modules/.astro/` 下，是刻意的
 *
 * 它是**能重建的派生物**（删了下次自己画回来），和字形子集缓存同一个地方。
 * 丢了会让人重复做某件事的那种记录（比如雪球草稿箱的流水账 `.xueqiu-drafts.json`）
 * **刚好相反**，放在仓库根。两者别放一块。
 */
...
/**
 * 缓存文件名，**纯 ASCII**。
 * （最早是因为小红书那个发帖工具要从磁盘读这一份，而它的排障清单里「路径带中文」是
 *   发布失败的已知原因。那一套删了，ASCII 照留 —— 一个缓存文件名没有理由带中文。
 *   `/_cards` 给人下载的那个名字 `牢玩家-r1009.png` 是另一回事，只在浏览器里用。）
 */
