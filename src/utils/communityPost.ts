 ...
 * `agentProductName()`、`xhsPost.ts` 差点抄它的标签拼法），所以这一条要写明白：
 *
 *   |  | X（`sharePost.ts`） | 小红书（`xhsPost.ts`） | 雪球 / 富途（这里） |
 *   |---|---|---|---|
 *   | 总量 | 280 **加权** | 标题 20 ＋ 正文 1000 | **长文原生**，雪球标题无上限 |
 ...
 *   差的只有两处参数（代码怎么写、标题有没有上限），那是 `SocialPlatform`
...
/** 组里的一份：谁写的 ＋ 它那段摘要。形状和 `XhsPostVoice` 一样，刻意不复用类型 —— */
...
export interface CommunityPost {
   ...
   * ★ 单独一格而不是直接截：雪球没上限，富途那个 80 是**猜的**
   *   （`calibrated: false`）—— 拿一个没核过的数去截真标题，是把推测
   *   当成事实在改内容。这儿只报，让人自己看着办。
   */
  titleOver?: number;
}
 ...
 *   和 `xhsPost.ts` 的 `xhsTitle()` 结论相同，**理由却不同**，所以各写一份：
 *   那边是 20 个字的预算问题（一个代码把整格占了），这边**根本没有预算问题**
 *   （雪球标题无上限）—— 这边的理由是**信息量**：一篇叫「ZM」的帖子谁也看不出
 ...
 * 生成一篇。
 *
 * ★ 空数组不兜底（同 `xhsPost()` / `xPostText()`）：兜一个"没有出处的单条"
